...
/**
Corrected Approach.
1. Added double boundary checks using && operator to properly shift the boundaries (start or end pointer).
*/
...
function searchTarget(arr, target) {
  ...
  while (start <= end) {
    ...
    if (arr[mid] >= arr[start]) {
      ...
      // No need for target <= arr[mid] check as target == arr[mid] check has been done initially in an early return condition.
      if (target >= arr[start] && target < arr[mid]) {
        // To **ensure** target lies within the left sorted half.
        // Double check is a mandate.
      ...
      if (target <= arr[end] && target > arr[mid]) {
        // Now, above if-condition ensures that target lies within the RHS.
        // So, shift start-pointer accordingly.
        start = mid + 1;
      } else {
        // else-if (target < arr[mid])
