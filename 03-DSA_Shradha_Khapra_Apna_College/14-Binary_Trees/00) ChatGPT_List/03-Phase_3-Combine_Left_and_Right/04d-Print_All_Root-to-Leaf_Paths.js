...
const printPaths = (root, res = [], path = []) => {
  // Universal Base Case for Binary Trees:
  if (!root) return; // Return from the point when there is no root available anymore in the tree.

  // Push current valid node right away before recursing anymore/further.
  path.push(root.val);
