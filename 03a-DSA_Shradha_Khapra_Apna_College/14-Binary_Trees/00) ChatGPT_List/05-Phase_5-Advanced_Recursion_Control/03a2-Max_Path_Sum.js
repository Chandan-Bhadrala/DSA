...
/**
## Improvement: In Approach.
1. Using Kadane's logic.
2. As I've to return **max possible sum**, so I've to exclude the paths/subtree which contributes negatively.
*/
...
function maxPathSum(root) {
  ...
  function pathSum(root) {
    ...
    let ltSum = Math.max(pathSum(root.left), 0); // Collecting the sum of the left branch.
    let rtSum = Math.max(pathSum(root.right), 0); // Collecting the sum of the right branch.
